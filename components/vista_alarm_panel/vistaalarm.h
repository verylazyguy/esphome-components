...
#define ESP_LOGW(tag, fmt, ...)   Serial.printf(fmt "\n", __VA_ARGS__)
#define ESP_LOGI(tag, fmt, ...)   Serial.printf(fmt "\n", __VA_ARGS__)
#define ESP_LOGD(tag, fmt, ...)   Serial.printf(fmt "\n", __VA_ARGS__)
#define ESP_LOGV(tag, fmt, ...)   Serial.printf(fmt "\n", __VA_ARGS__)
#define ESP_LOGVV(tag, fmt,  ...) Serial.printf(fmt "\n", __VA_ARGS__)
#endif
...
static const char *const TAG = "vista";
...
int getZoneFromPrompt(char *p1) {
  ...
  if (debug > 1)
    ESP_LOGD(TAG,"The prompt was matched - vista zone is %d",vista.statusFlags.zone);
  char s[5];
  x++;
  for (y=0;y<4;y++) {
    ...
    if (p1[y+x]==0x20 && y>0) {
      ...
      zone=z;
      if (debug > 2)
        ESP_LOGVV(TAG,"The zone match is: %d",zone);
      break;
...
bool promptContains(char * p1, const char * msg, int & zone) {
  ...
  if (debug > 1)
    ESP_LOGD(TAG,"The prompt  %s was matched - vista zone is %d",msg,vista.statusFlags.zone);
  char s[5];
  x++;
  for (y=0;y<4;y++) {
    if (p1[y+x] > 0x2F && p1[y+x] < 0x3A)
...
void printPacket(const char * label, char cbuf[], int len) {
  ...
  ESP_LOGVV(TAG, "%s: %s", label, s.c_str());
}
